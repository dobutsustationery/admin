...
exports.createOrder = functions.https.onRequest(async (req, res) => {
	try {
		...
		const path = `${ROOT_COLLECTION}/${jsonResponse.id}`;
		await db
			.doc(path)
			.set({ order_request: req.body, order_response: jsonResponse });
		res.status(httpStatusCode).json(jsonResponse);
	} catch (error) {
		console.error("Failed to create order:", error);
		res.status(500).json({ error: "Failed to create order." });
		const db = admin.firestore();
		const path = `${ROOT_COLLECTION}`;
		await db.collection(path).doc().set({ order_request: req.body, error });
	}
});
...
exports.capture = functions.https.onRequest(async (req, res) => {
	...
	try {
		...
		const db = admin.firestore();
		const path = `${ROOT_COLLECTION}/${orderID}`;
		await db.doc(path).update({ order: req.body, payment: jsonResponse });
		res.status(httpStatusCode).json(jsonResponse);
	} catch (error: any) {
		console.error("Failed to create order:", error);
		res.status(500).json({ error: "Failed to capture order." });
		const db = admin.firestore();
		const path = `${ROOT_COLLECTION}_errors/${orderID}`;
		await db.doc(path).update({ error });
	}
});
