...
describe.skipIf(!isLiveConfigured)('Google Drive Integration (@live)', () => {
    ...
    it('should list files in the sandbox', async () => {
        ...
        const found = await GoogleDrive.findFolder('NonExistent', sandboxFolderId, accessToken);
        expect(found).toBeNull();
    });

    it('should create a subfolder', async () => {
        const subId = await GoogleDrive.createFolder('SubTest', sandboxFolderId, accessToken);
        expect(subId).toBeDefined();
        
        // Verify it exists
        const foundId = await GoogleDrive.findFolder('SubTest', sandboxFolderId, accessToken);
    ...
    it('should upload and download a file', async () => {
        ...
        const fileInfo = await GoogleDrive.uploadImageToDrive(blob, filename, sandboxFolderId, accessToken);
        ...
        // Test Download
        const downloadedContent = await GoogleDrive.downloadFile(fileInfo.id, accessToken);
    ...
    it('should set file permissions', async () => {
         ...
         await expect(GoogleDrive.setFilePermissions(fileInfo.id, accessToken)).resolves.not.toThrow();
    });
});
