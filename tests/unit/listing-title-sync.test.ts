...
describe("Listing Title Sync", () => {
  it("synchronizes item description with listing title on link and updates", () => {
    ...
    state = rootReducer(
      state,
      update_item({ id: "item-a", item: itemA as any }),
    );
    ...
    state = rootReducer(
      state,
      update_listing({ handle: "listing-l", changes: { title: "New Title" } }),
    );
    ...
    state = rootReducer(
      state,
      update_item({ id: "item-b", item: itemB as any }),
    );
    state = rootReducer(
      state,
      update_field({
    ...
    expect(state.inventory.idToItem["item-a"].description).toBe(
      "Changed Title",
    );
    expect(state.listings.handleToListing["listing-l"].title).toBe(
      "Changed Title",
    ); // FAIL expected
    expect(state.inventory.idToItem["item-b"].description).toBe(
      "Changed Title",
    ); // FAIL expected
  });
});
