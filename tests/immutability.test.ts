...
import { inventory, initialState as inventoryInitialState } from "$lib/inventory";
...
function generateRandomAction(state: any, actionNumber: number) {
  ...
  switch (actionType) {
    ...
    case "update_field": {
      ...
      const field = ["qty", "shipped", "description"][Math.floor(Math.random() * 3)] as keyof Item;
...
describe("reducer immutability", () => {
  it("verifies that prior states remain unchanged after actions are dispatched", () => {
    ...
    expect(snapshotCount).toBeGreaterThanOrEqual(totalActions / snapshotInterval);
  ...
  it("verifies immutability by comparing old state snapshots to reconstructed states", () => {
    // This test uses a different approach: we'll track the actual state objects
    // and verify they don't change
    const store = configureStore({
      reducer: {
        names,
        inventory,
