...
function main() {
  ...
  if (!platformPackage) {
    console.warn(
      `Warning: No Biome CLI package available for ${os.platform()}-${os.arch()}`,
    );
  ...
  console.log(
    `Installing ${platformPackage}@${BIOME_VERSION} for your platform...`,
  );

  try {
    execSync(
      `npm install --no-save --ignore-scripts ${platformPackage}@${BIOME_VERSION}`,
      {
        stdio: "inherit",
