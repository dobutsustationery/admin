...
- **Recommendation:** Consider if the `sync_requested` event should only contain the _intent_ (the handle) and let the processor (Worker/Function) resolve the latest state from Firestore before executing the sync. However, the current "snapshot at request time" approach is valid for "Intent" if we want to ensure we sync what the user _saw_ at that moment.
