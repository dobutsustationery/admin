...
The user's intuition that this *could* be done in the browser is technically understandable for modern web apps, but in the context of the Shopify Admin API, a backend (Cloud Function or CLI) is **the only secure and functional way** for several reasons:
...
While a backend is mandatory, the *type* of backend could vary:
...
- **Trigger Efficiency:** The Cloud Function triggers on *every* broadcast action. As the system grows, this will become expensive. It would be better to use a specific sub-collection for tasks or a more granular trigger.
