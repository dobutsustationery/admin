...
Every image file created by the application in Google Drive will be tagged with a `properties.derivation_key` that uniquely identifies the _intent_ that created it. (We use `properties` rather than `appProperties` for access-control reasons — `properties` are visible across apps and in the Drive UI, which is needed for cross-service queries.)
