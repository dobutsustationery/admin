...
3.  **Root Reducer Execution**:
    ...
    - **Step A**: It reads the _current_ `orderImport.rows` state.
    - **Step B**: It executes the logic to parse those rows (using current code).
    - **Step C**: It constructs a _synthetic, internal-only_ action: `bulk_import_items` (containing the parsed items).
