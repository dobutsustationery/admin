...
Currently, `src/lib/gemini-client.ts` _detects_ subtypes during "Generate Descriptions" and updates the local UI, but it **does not** update the Redux state (`janCodeToPhotos`). This means `generate_proposals` never sees the `JAN:Subtype` keys.
...
- **Result**: The Inventory is already split _before_ Listing Creation.
