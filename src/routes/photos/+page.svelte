<script lang="ts">
  ...
  async function handleGenerate() {
    ...
    try {
      const token = await import("$lib/google-photos").then((m) =>
        m.getStoredToken(),
      );
      ...
      if (
        ...
        handleDisconnect();
        error =
          "Authentication expired or insufficient permissions. Please connect again to grant access.";
      }
    } finally {
      isGenerating = false;
          ...
          <div class="space-y-8">
            {#each analysisGroups as group}
              ...
              >
                ...
                <!-- Thumbnails Row -->
                <div
                  class="flex flex-row flex-wrap gap-4 mt-4 mb-6"
                ...
                {:else}
                  ...
                  >
                    <span class="animate-pulse"
                      >Waiting for AI description...</span
                    >
                  </div>
                {/if}
              </div>
