<script lang="ts">
  ...
  async function fetchSafeDataUrl(url: string): Promise<string> {
    ...
    if (isDriveApi) {
      if (!token) {
        throw new Error(
          "Authentication missing. Please Switch Account to grant Drive permission.",
        );
      }
  ...
  async function uploadBlob(blob: Blob, suffix: string) {
    ...
    try {
      ...
      if ($user.uid) {
        ...
        console.warn(
          "[Manual Upload] No User UID! Dispatching locally only. Persistence will fail.",
        );
        store.dispatch(action);
      }
    } catch (e: any) {
...
<div class="min-h-screen bg-gray-50 flex flex-col">
  ...
  <main class="flex-1 p-8 max-w-6xl mx-auto w-full">
    ...
    {#if !photoId}
      <div
        class="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-md flex flex-col items-center gap-4"
      >
    ...
    {:else if !item && urlHistory.length === 0}
      <div
        class="bg-red-50 border border-red-200 text-red-800 p-4 rounded-md flex flex-col items-center gap-4"
      >
    ...
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div
          class="md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6"
        >
          ...
          {#if effectiveHistory.length > 0}
            ...
            <div
              class="mt-4 text-xs text-gray-500 text-center w-full max-w-full overflow-hidden"
            >
          ...
          <div class="mt-8 pt-6 border-t border-gray-100">
            ...
            <p class="text-sm text-gray-500 mb-4">
              Upload an alternate image. This will become the new current active
              image for this photo ID.
            </p>
        ...
        <div
          class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-fit"
        >
          ...
          <div class="space-y-6 relative border-l-2 border-slate-200 ml-3">
            {#each effectiveHistory as url, i}
              <div class="relative flex items-start pl-6 group">
                <!-- Dot -->
                <div
                  class="absolute -left-[9px] top-4 flex items-center justify-center w-5 h-5 rounded-full border-2 border-white bg-slate-300 group-first:bg-indigo-600 shadow z-10"
                ></div>

                <!-- Content -->
                <div
                  class="ml-4 w-full p-4 rounded-lg border border-gray-200 bg-white shadow-sm"
                >
                  ...
                  <div
                    class="text-xs text-gray-500 mb-2 w-full max-w-full overflow-hidden"
                  >
                  ...
                  <div class="flex flex-wrap gap-2 mt-3">
                    ...
                    >
                      {#if processingOp === url + "_smart_crop"}...{:else}Auto
                        Crop{/if}
                    </button>
                    <button
