<script lang="ts">
  ...
  $: {
    ...
    for (const entry of logs) {
      ...
      if (
        tempGroups.length > 0 &&
        Math.abs(tempGroups[tempGroups.length - 1].timestamp - ts) < 2
      ) {
  ...
  $: filteredGroups = filter
    ? groupedLogs.filter(
        (g) =>
          ...
          g.children.some((c) =>
            c.action.type.toLowerCase().includes(filter.toLowerCase()),
          ),
      )
    : groupedLogs;
            ...
            {#each group.children as child (child.id)}
              ...
              >
                <div class="content">
                  ...
                  <div class="type" title={child.action.type}>
                    {child.action.type}
                  </div>
                </div>
              </div>
