<script lang="ts">
  ...
  $: inventoryItems = (
    Object.entries($store.inventory.idToItem) as [string, Item][]
  ).map(([key, item]) => {
    ...
    if (listing && item.image) {
      const matchingImg = listing.images.find(
        (img: ListingImage) => img.url === item.image,
      );
      if (matchingImg) {
  ...
  function commitEdit(id: string, field: string, value: any, index: number) {
    ...
    if (["description", "bodyHtml", "productCategory"].includes(field)) {
      if (listing) {
        ...
        console.warn(
          `Attempted to edit listing field ${field} for ${handle} but no listing exists.`,
        );
      }
      ...
      if (listing && item.image) {
        const newImages = (listing.images || []).map((img: ListingImage) => {
          if (img.url === item.image) {
            return {
              ...
              position:
                field === "imagePosition" ? Number(value) : img.position,
            };
          }
          return img;
        });
        broadcast(
          firestore,
          $user.uid,
          update_listing({ handle, changes: { images: newImages } }),
        );
      }
    } else {
      // Item Field
  ...
  function downloadCSV() {
    ...
    Object.entries(rowsByHandle).forEach(([handle, variants]) => {
      ...
      variants.forEach((v, idx) => {
        if (v.image) {
          const matchIdx = rawGallery.findIndex(
            (img, i) => img.url === v.image && !usedGalleryIndices.has(i),
          );
          if (matchIdx !== -1) {
      ...
      const remainingImages = rawGallery.filter(
        (_, i) => !usedGalleryIndices.has(i),
      );
      ...
      for (let i = 0; i < maxRows; i++) {
        ...
        if (i === 0) {
          ...
          exportRow["Product Category"] = listing
        ...
        if (variant) {
          ...
          exportRow["Variant SKU"] = generateSku(
            variant.janCode,
            variant.subtype,
          );
