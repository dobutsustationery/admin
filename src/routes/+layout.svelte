  ...
  onMount(async () => {
    ...
    console.error = (...args: any[]) => {
      ...
      if (errorStr.includes("Component auth has not been registered yet")) {
        ...
        console.log(
          `⚠️  Suppressed transient auth initialization error (${suppressedErrorCount})`,
        );
        return;
      }
    ...
    setTimeout(() => {
      ...
      if (suppressedErrorCount > 0) {
        console.log(
          `✓ Suppressed ${suppressedErrorCount} transient auth errors during initialization`,
        );
      }
    }, 1000);
