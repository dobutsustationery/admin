...
export const driveSyncMiddleware: Middleware<{}, any> = storeAPI => next => async action => {
    ...
    if (action.type === 'photos/categorize_photo') {
        ...
        // Check if uploaded
        const isUploaded = state.photos.uploads[photo.id]?.status === 'completed' || 
        ...
        if (isUploaded) {
            ...
            if (token) {
                try {
                    let fileId = "";
                    const driveUrl = photo.baseUrl || state.photos.urlHistory[photo.id]?.[0];
                    if (driveUrl) {
                    ...
                    if (fileId) {
                        ...
                        renameFile(fileId, newName, token.access_token).catch((e: any) => console.error("Drive Rename Failed", e));
                    }
                } catch (e) {
             ...
             if (token) {
                 try {
                    ...
                    if (fileId) {
                        ...
                        const ext = item ? (item.filename.split('.').pop() || 'jpg') : 'jpg';
                        const safeId = id.replace(/[^a-zA-Z0-9-_]/g, '');
                        const newName = `${janCode}_${safeId}.${ext}`;
                        
                        console.log(`[DriveSync] Renaming completed upload ${fileId} to ${newName}`);
                        renameFile(fileId, newName, token.access_token).catch((e: any) => console.error("Drive Rename Failed", e));
                    }
