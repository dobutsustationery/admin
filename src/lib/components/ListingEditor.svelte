<script lang="ts">
  ...
  $: isCategoryValid = !!(
    ...
    categorySet.has(listing.productCategory)
  );
  ...
  $: if (
    listing &&
    listing.productCategory !== undefined &&
    !isCategoryInteracting
  ) {
  ...
  $: uniqueAssociatedItems = Array.from(
    new Map(
      associatedItems.map((item) => [item.variantId || item.id, item]),
    ).values(),
  );
  ...
  $: displayedSubtypes = (() => {
    ...
    const byId = new Map(
      uniqueAssociatedItems.map((item) => [item.variantId || item.id, item]),
    );
    const ordered: any[] = [];
  ...
  $: mainImageObj = (() => {
    ...
    if (selectedSubtypeId) {
      const item = associatedItems.find(
        (i) => (i.variantId || i.id) === selectedSubtypeId,
      );
      if (item) {
        // 1. Explicit Override
        if (item.variantImage) {
          ...
          return (
            existing || {
              ...
              altText: item.subtype,
              id: "subtype-ovr-" + (item.variantId || item.id),
              position: -1,
            }
          );
        }
        ...
        if (item.photoGroupKey) {
          const groupImg = images.find(
            (img) => img.sourceGroup === item.photoGroupKey,
          );
        ...
        if (item.image) {
          ...
          return (
            existing || {
              url: item.image,
              altText: item.subtype,
              id: "subtype-inv-" + (item.variantId || item.id),
              position: -1,
            }
          );
        }
  ...
  function handleSubtypeDragOver(e: DragEvent, targetId: string) {
    ...
    if (!targetId || targetId === draggingSubtypeId || !previewSubtypeOrderIds)
      return;
  ...
  $: price =
    associatedItems.length > 0 && associatedItems[0].price
  ...
  <div class="listing-content">
        ...
        >
              ...
              {#if !readOnly}
                <div class="thumb-overlay">
                  <button
                    ...
                    on:click={(e) => deleteImage(e, img)}
                    title="Delete"
                  >
                    ✕
                  </button>
                  <button
                    class="overlay-btn upload"
                    on:click={(e) => triggerReplace(e, img)}
                    title="Replace"
                  >
                    ⬆
                  </button>
          ...
          {#if !readOnly}
            <div class="thumbnail-container">
              <button
                class="thumbnail-btn add-btn"
                on:click={() => dispatch("addImage")}
                title="Add Image"
              >
            ...
            <div class="subtype-row">
              ...
              {#if isCreationMode}
                ...
                <input
                  ...
                  title={totalAlloc > avail
                    ? `Over-allocated! ${totalAlloc}/${avail}`
                    : totalAlloc < avail
                      ? `Under-allocated: ${totalAlloc}/${avail} (remainder stays on source)`
                      : "Fully allocated"}
                />
              ...
              {#if subtypeImg}
                <div class="subtype-thumb-wrapper">
                  <button
                    class="subtype-thumb-btn {selectedSubtypeId ===
                    ...
                    on:click={() =>
                      !readOnly
                        ? dispatch("replaceSubtypeImage", item)
                        : handleSubtypeSelect(item.variantId || item.id)}
                  ...
                  {#if !readOnly}
                    <div class="subtype-overlay">
                      <button
                        class="overlay-btn delete"
                        on:click={(e) => deleteSubtypeImage(e, item)}
                        title="Unlink Image"
                      >
                        ✕
                      </button>
                      <button
                        class="overlay-btn upload"
                        on:click={(e) => triggerSubtypeReplace(e, item)}
                        title="Replace Image"
                      >
                        ⬆
                      </button>
              ...
              {:else}
                <button
                  ...
                  on:click={() =>
                    !readOnly ? dispatch("replaceSubtypeImage", item) : null}
    ...
    <div class="details-column">
      <div class="title-block">
        ...
        <div
          class="listing-price-container {!isPriceValid && !readOnly
            ? 'invalid'
            : ''}"
        >
          ...
          {#if readOnly}
            <span class="price-val"
              >{associatedItems[0]?.price?.toFixed(2) || "0.00"}</span
            >
          {:else}
            <input
              type="number"
      ...
      <div class="category-block">
        ...
        {#if readOnly}
          <span class="category-val"
            >{listing.productCategory || "Uncategorized"}</span
          >
        {:else}
          <div
            class="category-input-wrapper {!isCategoryValid || !!categoryError
              ? 'invalid'
              : ''}"
                ...
                {:else}
                  {#each filteredCategories as cat}
                    ...
                    <div
                      ...
                      on:keydown={(e) =>
                        e.key === "Enter" && selectCategory(cat)}
                      tabindex="0"
                    >
                      {cat}
                    </div>
          ...
          <div class="options-list" role="list">
            {#each displayedSubtypes as item (item.variantId || item.id)}
              <button
                ...
                on:dragover={(e) =>
                  handleSubtypeDragOver(e, item.variantId || item.id)}
      ...
      {:else}
        ...
        <div class="actions-block">
          ...
          <button class="btn-buy-shop"
            >Buy with <span class="shop-logo">shop</span></button
          >
          <div class="payment-options">
            <button class="payment-link" type="button"
...
<style>
  ...
  .thumbnail-container[draggable="true"] {
    cursor: grab;
  }
  .thumbnail-container[draggable="true"]:active {
    cursor: grabbing;
  }
  ...
  .thumbnail-btn :global(img) {
    pointer-events: none;
  }
  :global(.thumbnail-btn.selected) {
    border-color: #3b82f6;
  ...
  .thumbnail-container:hover .thumb-overlay {
    opacity: 1;
  }
  .overlay-btn {
    background: none;
  ...
  .subtype-qty-input.over-allocated {
    border-color: #ef4444;
    color: #b91c1c;
    background-color: #fef2f2;
  }
  .subtype-qty-input.under-allocated {
    border-color: #22c55e;
    color: #15803d;
    background-color: #f0fdf4;
  }
  .subtype-qty-input.valid-allocated {
    border-color: #3b82f6;
    color: #1e40af;
