...
const SCOPES = (import.meta.env.VITE_GOOGLE_DRIVE_SCOPES || 'https://www.googleapis.com/auth/drive.file').split(',');
...
export function isDriveConfigured(): boolean {
  return !!(CLIENT_ID && FOLDER_ID && CLIENT_ID !== 'your-client-id.apps.googleusercontent.com');
}
...
export function initiateOAuthFlow(): void {
  if (!isDriveConfigured()) {
    console.error('Google Drive is not configured. Please set VITE_GOOGLE_DRIVE_CLIENT_ID and VITE_GOOGLE_DRIVE_FOLDER_ID');
    return;
  }
 ...
 */
export async function listFilesInFolder(accessToken: string): Promise<DriveFile[]> {
  if (!FOLDER_ID) {
