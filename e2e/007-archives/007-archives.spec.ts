...
/**
 * E2E test for the /archives page
 *
 ...
 * - 002-xxx: Archives page loaded with form
 *
...
test.describe("Archives Page", () => {
   ...
   */
  test("complete archives workflow", async ({ page, context }) => {
    ...
    await screenshots.capture(page, "signed-out-state", {
      programmaticCheck: async () => {
        ...
        if (headingVisible) {
          ...
          expect(headingText).toContain("Archives");
        }
      },
    });
    ...
    await archiveInput.waitFor({ state: "visible", timeout: 5000 });
    console.log("   ✓ Archive input found");

    await screenshots.capture(page, "archives-loaded", {
      programmaticCheck: async () => {
        // Verify heading
        await expect(page.locator("h1")).toContainText("Archives");
        console.log("   ✓ Page heading shows 'Archives'");

        // Verify archive input field
        await expect(archiveInput).toBeVisible();
        console.log("   ✓ Archive name input is visible");

        // Verify Add Archive button
        const addButton = page.locator('button:has-text("Add Archive")');
        await expect(addButton).toBeVisible();
        console.log("   ✓ Add Archive button is visible");

        // Verify list element exists (may be empty initially)
        const archivesList = page.locator("ul");
        const listVisible = await archivesList.isVisible().catch(() => false);
        if (listVisible) {
          console.log("   ✓ Archives list is visible");
        }
    ...
    console.log("\n✅ Complete archives workflow test passed!");
