...
test.skip("Debug Gemini Image Editing", async ({ page }) => {
  ...
  if (!apiKey) {
    console.error(
      "CRITICAL: No API Key found in .env, .env.local, or .env.staging",
    );
    return;
  }
  ...
  const imagePath = path.resolve(
    "e2e/test-images/006ccee443ad388aa0799f9d6d97290f.jpg",
  );
  ...
  const strategies = [
    ...
    },
    {
      ...
      prompt:
        "Remove the background from this image. Then, crop the image tightly around the subject with a 15px margin. Return ONLY the raw Base64 encoded string of the resulting PNG image. Do not use Markdown formatting. Do not wrap in JSON.",
      mimeType: "text/plain",
      maxTokens: 65536,
    },
    {
      ...
      prompt:
        'Remove background. Return JSON object { "image_data": "<BASE64>" }.',
  ...
  for (const strat of strategies) {
    ...
    const result = await page.evaluate(
      async ({ apiKey, strat, base64Image }) => {
    ...
    if (result.success) {
      ...
      candidate?.content?.parts?.forEach((p: any, i: number) => {
        if (p.inline_data) {
          console.log(
            `  Part ${i}: [INLINE IMAGE] ${p.inline_data.mime_type}, Size: ${p.inline_data.data.length} chars`,
          );
          foundImage = true;
        } else if (p.text) {
          const snippet = p.text.substring(0, 100).replace(/\n/g, " ");
          console.log(
            `  Part ${i}: [TEXT] Length: ${p.text.length} chars. Start: "${snippet}"`,
          );
