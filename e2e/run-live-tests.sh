...
# Live E2E Test Runner Script
# This script sets up and runs the Playwright Live E2E tests with Firebase emulators on fresh ports
...
EMULATOR_LOG_PATH="${E2E_EMULATOR_LOG_PATH:-/tmp/dobutsu-live-e2e-emulators.log}"
...
' ' ')"
    ...
    for pid in ${pids}; do
      ...
      if [ "${comm_base}" = "java" ] || [ "${comm_base}" = "node" ]; then
        echo "Killing stray ${comm_base} process ${pid} on emulator port ${port}..."
        kill "${pid}" 2>/dev/null || true
        sleep 1
        if lsof -t -nP -iTCP:"${port}" -sTCP:LISTEN 2>/dev/null | grep -q "^${pid}$"; then
          echo "Force killing stubborn process ${pid} on emulator port ${port}..."
...
# Always start fresh emulators for live tests
...
echo ""
echo "▶️  Running Playwright Live tests..."
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
  ...
  echo "❌ Some live tests failed (exit code: $TEST_EXIT_CODE)"
  echo ""
