...
test.describe("Inventory Receipt with Google Drive", () => {
  ...
  test("complete Inventory Receipt workflow (Match, Conflict, New)", async ({
    page,
    authenticatedPage,
  }, testInfo) => {
    // --- Setup Helpers ---
    ...
    const MOCK_CSV = `JAN CODE,TOTAL PCS,DESCRIPTION,Carton Number
...
`,
    ...
    await expect(
      page.locator('h1:has-text("Inventory Receipt (Drive)")'),
    ...
    await flow.step("View File List", "view-files", [
      {
        ...
        check: async () =>
          await expect(page.locator("text=Connected to Drive")).toBeVisible(),
      },
      {
        description: "CSV file should be listed",
        check: async () =>
          await expect(page.locator("text=receipt-2025.csv")).toBeVisible(),
      },
    ]);
    ...
    await expect(page.locator(".file-list button").first()).toHaveClass(
      /selected/,
    );
    ...
    await flow.step("Verify Preview", "verify-preview", [
      {
        ...
        check: async () =>
          await expect(page.locator("text=Sync: 0")).toBeVisible({
            timeout: 10000,
          }),
      },
      {
        ...
        check: async () =>
          await expect(
            page.locator('h2:has-text("Preview: receipt-2025.csv")'),
          ).toBeVisible(),
      },
      {
        ...
        check: async () => {
          await expect(
            page.locator('button:has-text("Process Matches")'),
          ).toBeVisible();
          await expect(
            page.locator('button:has-text("ADD to inventory")'),
          ).toBeVisible();
        },
      },
      {
        ...
        check: async () => {
          await expect(
            page.locator('tr:has-text("1010101010101")'),
          ).toBeVisible({ timeout: 10000 });
          await expect(
            page.locator('tr:has-text("1010101010101")'),
          ).toContainText("ADD");
      ...
      },
      {
        ...
        check: async () => {
          // Conflict item
          await expect(
            page.locator('tr:has-text("4510085530713")'),
      ...
      },
      {
        ...
        check: async () => {
          // Should be MATCH
          await expect(
            page.locator('tr:has-text("4542804044355")'),
          ).toContainText("MATCH");
        },
      },
    ]);
    ...
    await flow.step("Process New Items", "process-new", [
      {
        ...
        check: async () => {
          ...
          await expect(
            page.locator('tr:has-text("New Mystery Item")'),
          ).toBeHidden();
          // Wait for success message to disappear for consistent screenshots
          await expect(page.locator(".success-message")).toBeHidden();
        },
    ...
    await flow.step("Confirm Conflict Resolution", "005-conflict-resolved", [
      {
        ...
        check: async () => {
          ...
          await expect(
            page
              .locator('tr:has-text("4510085530713")')
