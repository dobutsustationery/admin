...
test.describe("SKU Review - Unlisted Items", () => {
  ...
  test("should flag unlisted items with UNLISTED badge", async ({
    ...
    await test.step("Inject Test Data", async () => {
      await page.evaluate(async () => {
        ...
        if (!helpers)
          throw new Error(
            "testHelpers not found on window. Ensure VITE_FIREBASE_ENV=local",
          );
          ...
          bulk_import_items({
            items: [
              // Explicitly link the listed item to the handle we created above
              {
                type: "new",
                id: listedItem.id,
                item: { ...listedItem, handle: "test-listed-item" },
              },
              {
                type: "new",
                id: unlistedItem.id,
                item: { ...unlistedItem, handle: "test-unlisted-item" },
              },
            ],
          }),
        );
    ...
    await test.step("Verify Unlisted Badge", async () => {
      ...
      await expect(
        unlistedRow.locator(".badge", { hasText: "Unlisted" }),
      ).toBeVisible();

      // Verify the listed one does NOT have it
      await expect(
        listedRow.locator(".badge", { hasText: "Unlisted" }),
      ).toBeHidden();
    });
  });
});
