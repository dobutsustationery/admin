...
test.describe("JAN Codes Page", () => {
  ...
  test("complete jancodes workflow", async ({ page, context }, testInfo) => {
    ...
    const step1Verifications = [
      ...
      {
        description: 'Validated heading contains "Items with a blank subtype"',
        check: async () => {
    ...
    const step3Verifications = [
      ...
      {
        description: "Validated table headers (if visible)",
        check: async () => {
          const tables = page.locator("table");
          const count = await tables.count();
          if (count > 0) {
            const headers = await tables
              .first()
              .locator("thead th")
              .allTextContents();
