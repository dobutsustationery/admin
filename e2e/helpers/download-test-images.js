...
if (matchJancodesLimit) {
  console.log(
    `🔍 Extracting JAN codes from first ${matchJancodesLimit} records...`,
  );
  ...
  console.log(
    `   Filtering ${broadcastDocs.length} records by these JAN codes...`,
  );
  ...
  console.log(
    `🔍 Extracting image URLs from all ${broadcastDocs.length} broadcast records...`,
  );
...
for (const url of imageUrls) {
  try {
    ...
    execSync(`curl -s -m 5 -o "${localPath}" "${url}"`, { stdio: "pipe" });
...
const prevJson = existsSync(MAPPING_FILE)
