...
test.describe("Variant Updates (Modal-driven)", () => {
  test("User can split and remove variants in Draft mode with stable identity", async ({
    ...
    await page.evaluate(() => {
      ...
      store.dispatch = (action: any) => {
        if (
          action.type?.includes("requested") ||
          action.type?.includes("update_variant_qty")
        ) {
    ...
    // Wait for UI
    await expect(
      page.locator(".subtype-label", { hasText: "V1" }).first(),
    ...
    await page.waitForFunction(() =>
      (window as any).broadcastHistory.some((a: any) =>
        a.type.includes("add_variant_requested"),
      ),
    );
    const splitPayload = await page.evaluate(
      () =>
        (window as any).broadcastHistory.find((a: any) =>
          a.type.includes("add_variant_requested"),
        ).payload,
    );
    ...
    await page.waitForFunction(() =>
      (window as any).broadcastHistory.some((a: any) =>
        a.type.includes("remove_variant_requested"),
      ),
    );
    const removePayload = await page.evaluate(
      () =>
        (window as any).broadcastHistory.find((a: any) =>
          a.type.includes("remove_variant_requested"),
        ).payload,
  ...
  test("User can pick specific source item for split in Live mode", async ({
    authenticatedPage: page,
  }) => {
    test.setTimeout(120000);
    ...
    await page.evaluate(() => {
      ...
      const originalDispatch = store.dispatch;
      store.dispatch = (action: any) => {
        if (action.type?.includes("split_inventory_item"))
          (window as any).broadcastHistory.push(action);
        return originalDispatch(action);
      };
    });
        ...
        store.dispatch(
          actions.bulk_import_items({
            items: [
              {
                id: item1Id,
                item: { janCode, subtype: "S", qty: 5, handle },
                type: "new",
              },
            ],
          }),
    ...
    await expect(
      page.locator(".subtype-label", { hasText: "S" }).first(),
    ...
    await page
      .locator(".allocation-row", { hasText: "S" })
      .first()
    ...
    await page.waitForFunction(
      () => (window as any).broadcastHistory.length > 0,
    );
    const payload = await page.evaluate(
      () => (window as any).broadcastHistory[0].payload,
    );
