...
async function validatePermanentGoogleFixtures(): Promise<StaticFixtureConfig> {
  ...
  if (driveFolder.data.trashed) {
    throw new Error(
      `Configured Drive fixture folder is trashed: ${driveFolderId}`,
    );
  }

  const candidateParents = new Set<string>([driveFolderId]);
  const seedFolderId = await findDriveFolderIdByName(
    drive,
    driveFolderId,
    "Seed",
  );
  if (seedFolderId) candidateParents.add(seedFolderId);
  const imagesFolderId = await findDriveFolderIdByName(
    drive,
    driveFolderId,
    "Images",
  );
  if (imagesFolderId) {
    ...
    const originalsId = await findDriveFolderIdByName(
      drive,
      imagesFolderId,
      "Originals",
    );
    if (originalsId) candidateParents.add(originalsId);
    const processedId = await findDriveFolderIdByName(
      drive,
      imagesFolderId,
      "Processed",
    );
  ...
  if (!photosAccessToken) {
    throw new Error(
      "Could not obtain access token for Google Photos fixture validation.",
    );
  }
  ...
  sandboxId: async ({ page }, use) => {
    ...
    await page.addInitScript(
      (data) => {
        ...
        (window as any).__GOOGLE_DRIVE_SCOPES__ =
          "https://www.googleapis.com/auth/drive.file,https://www.googleapis.com/auth/photoslibrary.readonly";
