 ...
 * - 002-xxx: Order details page loaded
 *
 * Note: This page requires query parameters (orderId, email, product)
 * We'll test the page without parameters to verify basic functionality
 *
...
test.describe("Order Detail Page", () => {
  ...
  test("complete order detail workflow", async ({ page, context }) => {
    ...
    // ====================================================================
    // STEP 3: Wait for order page to be ready
    // ====================================================================
    console.log("\n📖 STEP 3: Wait for order page to be ready");
    ...
    await screenshots.capture(page, "order-page-ready", {
      programmaticCheck: async () => {
          ...
          try {
            ...
            if (store) {
              ...
              return {
                ...
                orderIdToOrder: state.inventory?.orderIdToOrder || {},
              };
            }
            return null;
          } catch (e) {
        ...
        if (orderState) {
          console.log(`   ✓ Redux order state present`);
          expect(orderState.hasInventory).toBe(true);
        }
    ...
    console.log("\n✅ Complete order detail workflow test passed!");
